...
Интерфейс: количество меняется нажатием на число. Кнопки `−` / `+` пока скрыты флагом `UI.qtySteppersEnabled` в `js/ui.js` (поставить `true`, чтобы снова показать).

Каждое изменение количества — одна запись в `change_history`. Ручной ввод числа фиксируется один раз (по Enter / потере фокуса). Нажатие на «Минимум:» справа от названия меняет порог критического остатка (`update_item_min_limit`).
...
Если GitHub Pages уже открыт, а пароль `1980` даёт «Нет связи с базой» / «База не обновлена», в SQL Editor повторно выполните **только** `schema.sql` и `policies.sql` (seed не запускать). Нужны актуальные RPC (`app_login`, `app_get_*`, `update_item_min_limit`, `create_note` с `p_assignee_ids`, `delete_daily_goal`); у `crypt()` должен быть виден schema `extensions`.
...
**Склад** — отделы на ПК стоят максимум в 3 колонки. Минимум справа от названия, его меняют только в админ-панели. В группе можно добавить позицию «сумма» — она показывает итог по остальным позициям группы и не редактируется вручную.

**План** — несколько целей на день. У каждой своя цифра и отслеживаемая позиция; факт берётся из количества этой позиции.

**Записки** — можно назначить нескольким сотрудникам. При входе на рабочий экран, если есть незакрытые записки на ваше имя, приходит оповещение (тост и красный бейдж на вкладке).
