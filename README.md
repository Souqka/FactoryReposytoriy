...
**История** — карточки изменений количества: сотрудник и цвет, позиция, производство → отдел, было/стало и разница. Фильтры по сотруднику, производству, отделу, позиции, периоду и поиск. Чтение только через `app_get_history`.

**Задачи** — вкладки «Мои» и «Все». «Мои задачи» показывают notes, назначенные текущему сотруднику из сессии. Счётчик незакрытых — бейдж на вкладке. Назначить можно нескольким сотрудникам.
