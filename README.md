...
Если GitHub Pages уже открыт, а пароль `1980` даёт «Нет связи с базой» / «База не обновлена», в SQL Editor повторно выполните **только** `schema.sql` и `policies.sql` (seed не запускать). Нужны RPC `app_login(p_password, p_client_key)`, `app_get_*` и `update_item_min_limit`; у `crypt()` должен быть виден schema `extensions`.
...
**Склад** — отделы (Запасы / Производство / Сборка и новые) на ПК стоят максимум в 3 колонки; если отделов меньше, они растягиваются на ширину экрана. На телефоне — одна колонка. Минимум у позиции справа от названия, его можно изменить нажатием.

**План** — «Цель на сегодня» сохраняется сразу (ввод / «Сохранить план»). Вместо подписи выбирается позиция: факт берётся из её количества и обновляется при изменениях на складе.
